...
<template>
  <div class="relative" ref="select-wrapper">
    <button class="p-2 pr-6 rounded-md border border-gray-200 focus:border-gray-400 dark:border-slate-800 dark:focus:border-slate-700 dark:bg-slate-800
            ...
            ref="select" @click="toggleOptions">
      ...
      <span>{{ selectedOption.label || selectedOption.value || 'Select a option' }}</span>
    ...
    <div
      ...
      <div class="flex flex-row items-center gap-2.5 px-2 py-2 cursor-pointer
                  dark:text-slate-300 font-['Nunito'] transition
                  bg-white dark:bg-slate-800 hover:bg-gray-100 dark:hover:bg-slate-700"
           v-for="(option, index) in options" :key="index"
           :class="{ 'bg-gray-200 dark:bg-slate-700 hover:bg-gray-200 dark:hover:bg-slate-700': option.value === selectedOption.value }"
      ...
      >
        ...
        <span class="leading-none text-sm">{{ option.label || 'No label' }}</span>
      </div>
    </div>
  </div>
</template>
